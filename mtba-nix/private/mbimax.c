/*-------------------------------------------------------------------------------------
 * Modified version of bimax.c by Eckart Zitzler
 * Changes required to make the program work under matlab environment were done.
 * (c)2013 Jayesh Kumar Gupta <rejuvyesh@gmail.com>
 *-------------------------------------------------------------------------------------*/
...
void  writeBicluster(int  firstRow, int  lastRow, cs_t  columnSet, int * x, int * y, int * z,int numBiclust,int * er)
{
...
void  conquer(int  firstRow, int  lastRow, int  level, int noMandatorySets, int32_T * x, int32_T * y, int32_T * z,int numBiclust, int32_T * er)
{
  int   overlapping;
  ...
  if (compareColumns(columnIntersection, consideredColumns[level],
    ...
    writeBicluster(firstRow, lastRow, columnIntersection,x,y,z,numBiclust,er);
  else {
    ...
    if (columnCount(consideredColumns[level + 1L]) >= minNoColumns &&
      ...
      if (noSelectedRows >= minNoRows)
        conquer(firstRow, firstRow + noSelectedRows - 1L, level + 1L, noMandatorySets,x,y,z,numBiclust,er);
    }
...
void  readInDataMatrix(double *p)
{
  int  i, j, cell;

  /* Matlab is mad; Columns and rows are inverted */
  for (i = 0L; i < noRows; i++) {
    for (j = 0L; j < noColumns; j++) {
      cell = (int)p[(j*noRows)+i];
      if (cell == 0)
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
  ...
  colLen = mxGetM(prhs[0]);

  data = mxGetPr(prhs[0]);
  

  noRows =  (int) mxGetScalar(prhs[1]); 
  noColumns = (int) mxGetScalar(prhs[2]); 
  minNoRows = (int) mxGetScalar(prhs[3]);
  minNoColumns = (int) mxGetScalar(prhs[4]);
  numBiclust = (int) mxGetScalar(prhs[5]);

  
  plhs[0] = mxCreateNumericMatrix(noRows,numBiclust,mxINT32_CLASS, mxREAL);
  plhs[1] = mxCreateNumericMatrix(numBiclust,noColumns,mxINT32_CLASS, mxREAL);
  plhs[2] = mxCreateNumericMatrix(noRows,noColumns,mxINT32_CLASS, mxREAL);
  plhs[3] = mxCreateNumericMatrix(1,1,mxINT32_CLASS, mxREAL);
  x = (int32_T *)mxGetData(plhs[0]);
  y = (int32_T *)mxGetData(plhs[1]);
  z = (int32_T *)mxGetData(plhs[2]);
  er = (int32_T *)mxGetData(plhs[3]);
  ...
  if (minNoColumns < 1L)
    minNoColumns = 1L;
  if (noColumns > 0L && noRows > 0L && initialize()){
    readInDataMatrix(data);
    conquer(0L, noRows - 1L, 0L, 0L, x, y, z, numBiclust, er); 
  }
