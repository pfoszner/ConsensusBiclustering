...
/******************************************************************/
void gibbssample(int nk,int nn, int pp, float **y,float 
  ...
  for (io=1;io<Ns;io++){
	...
	for (j1=0;j1<pp;j1++){
	  ...
	  if (flag){
	     m+=-y[i1][j1]*y[i1][j1]*tau_e[i]+log(tau_e[i]);
	  }
	}//i1
      }//j1
      m=m*0.5;
  ...
  for (i1=0;i1<nn;i1++){
	...
	flag=1;
	for (ik=0;ik<nk;ik++){
	  if ((int)sdelta[ik][i1]*(int)skapa[ik][j1]>0){
